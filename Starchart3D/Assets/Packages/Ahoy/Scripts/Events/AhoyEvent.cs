...
namespace Ahoy
{
	...
	public class AhoyEvent<T1, T2>
	{

		List<Action<T1, T2>> listeners;
		List<Action<T1, T2>> listenersOneShot;

		public AhoyEvent()
		{
			listeners = new List<Action<T1, T2>>();
			listenersOneShot = new List<Action<T1, T2>>();
		}

		public void Invoke(T1 val1, T2 val2)
		{
			//need to convert to array in case one is removed
			listeners.ToArray().ForEach(l => l(val1, val2));
			listenersOneShot.ToArray().ForEach(l => l(val1, val2));
			// listeners.ToArray().ForEach(l => l.Invoke(val));
			// listenersOneShot.ToArray().ForEach(l => l.Invoke(val));
			listenersOneShot.Clear();
		}
		...
		public void AddListenerOneShot(Action<T1, T2> listener)
		{
			listenersOneShot.TryAdd(listener);
		}
		public void RemoveListenerOneShot(Action<T1, T2> listener)
		{
			listenersOneShot.TryRemove(listener);
		}
